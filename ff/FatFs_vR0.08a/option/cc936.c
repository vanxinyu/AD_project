...
)
{
	...
	uint8_t unigbk[2];   // 暂存GBK高位与低位字节
	uint8_t gbkuni[2];   // 暂存UNICODE高位与低位字节
	...
	else 
	{
	   	if(dir == 0)  /* Unicode to OEMCP */
		{
			switch(src)
			{
				...
				{
					offset = ((((uint32_t)src - 0x4E00) * 2) + 0x0C0000);	/* 得到W25X16的UTG地址 */
					SPI_FLASH_BufferRead(unigbk,offset,2); /* 获取GBK码 */
					c = (((uint16_t)unigbk[0])<<8)+(uint16_t)unigbk[1];	 /* 把GBK码给了c */
				}
		...
		{
			GBKH=(uint8_t)(src>>8);  // 获取GBK高位字节
			GBKL=(uint8_t)(src);	   // 获取GBK低位字节
			GBKH-=0x81;
			GBKL-=0x40;
			offset=((uint32_t)192*GBKH+GBKL)*2;/* 得到W25X16的GTU地址 */
			SPI_FLASH_BufferRead(gbkuni,offset+0x0D0000,2);   /* 获取UNICODE码 */
			c = (((uint16_t)gbkuni[1])<<8)+(uint16_t)gbkuni[0];	 /* 把UNICODE码给了c */
		}
	}
	return c;
}
